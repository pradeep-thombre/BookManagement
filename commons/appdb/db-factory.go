...
import (
	...
	"book-management-api/commons/configs"
...
type AppDatabaseFactory interface {
	...
	NewDbConnection(dbname string) (DatabaseClient, error)
}
...
func (d *dbfactory) NewDBConnection(context context.Context) (DatabaseClient, error) {
	return d.NewDbConnection(os.Getenv(configs.MONGO_DATABASE))
}

// function to create a new connection, based on database name
func (d *dbfactory) NewDbConnection(dbname string) (DatabaseClient, error) {
